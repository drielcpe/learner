...
declare module "@tanstack/react-table" {
  interface TableMeta<TData extends unknown> {
    onStatusUpdate?: (paymentId: string, newStatus: PaymentStatus) => void
    isUpdating?: boolean
    showActions?: boolean
  }
}
...
const columns: ColumnDef<Payment>[] = [
  ...
  },
  {
    ...
    cell: ({ row, table }) => {
      ...
      const handleQRCode = () => {
        router.push(`/payments/qr/${payment.id}`)
      }

      // Get meta values from table
      const showActions = table.options.meta?.showActions
      const onStatusUpdate = table.options.meta?.onStatusUpdate
      ...
      return (
        <div className="flex gap-2">
          {/* Student actions */}
          {!showActions && payment.payment_method === "gcash" && (
            <Button 
              variant="outline" 
              size="sm" 
              className="h-8 gap-1"
          ...
          {showActions && (
            <>
              <Button 
                ...
                disabled={isUpdating || payment.status === 'processing' || payment.status === 'completed'}
              >
                Process
              </Button>
              <Button 
                ...
                disabled={isUpdating || payment.status === 'completed'}
              >
                Complete
              </Button>
              <Button 
